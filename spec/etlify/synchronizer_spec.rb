...
RSpec.describe Etlify::Synchronizer do
  ...
  context "sync_dependencies flush in rate-limited context" do
    it "syncs dependents inline instead of enqueuing when adapter has rate_limiter" do
      rate_limited_adapter = Etlify::Adapters::NullAdapter.new
      rate_limited_adapter.define_singleton_method(:rate_limiter) { @rl }
      rate_limited_adapter.define_singleton_method(:rate_limiter=) { |v| @rl = v }
      rate_limited_adapter.rate_limiter = Etlify::RateLimiter.new(
      ...
      # Dependent (user) should NOT receive crm_sync! (async)
      # but should be synced inline via Synchronizer.call
      expect(user).not_to receive(:crm_sync!)
      expect(Etlify::Synchronizer).to receive(:call)
        .with(company, crm_name: :hubspot).and_call_original
      expect(Etlify::Synchronizer).to receive(:call)
        .with(user, crm_name: :hubspot).and_return(:synced)
    ...
    it "enqueues dependents via crm_sync! when adapter has no rate_limiter" do
      Etlify::PendingSync.create!(
        dependent_type: "User",
        dependent_id: user.id,
        dependency_type: "Company",
